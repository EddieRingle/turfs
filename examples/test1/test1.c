...
int main(int argc, char **argv)
{
    ...
    turfs_fp fp;
    int ret = 0;
    turfs_init("turfsdev", "turfs", NULL);
    ...
    printf("Userdata (%d): %s\n", ret, test);
    free(test);
    ret = turfs_silph_scope("/userdata/subdir/test1.sav", &test, 0, NULL);
    if (ret) {
    ...
    free(test);
    ret = turfs_silph_scope("/assets/subdir/test1.txt", &test, 0, NULL);
    if (ret) {
    ...
    free(test);
    ret = turfs_silph_scope("/notreal/rofl.copter", &test, 0, NULL);
    if (ret) {
